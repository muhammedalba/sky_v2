...
const ForgotPasswordFlow = ({ locale }: { locale: string }) => {
  ...
  const [submittedCode, setSubmittedCode] = useState<string>(() => {
    if (typeof window !== "undefined") {
      return sessionStorage.getItem("reset-code") || "";
    }
    return "";
  });
  ...
  return (
    <div className="w-full pt-8">
      <AuthMobileLogo
        subtitle={t("forgotPasswordTitle")}
        className="lg:hidden"
      />
      <AuthHeader
        description={headerText[step]}
      ...
      <div className=" ">
        {step === "REQUEST" && <RequestStep onSuccess={handleRequestSuccess} />}
        ...
        {step === "RESET" && (
          <ResetStep email={submittedEmail} passwordResetCode={submittedCode} />
        )}
      </div>
