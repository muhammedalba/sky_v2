...
export const authApi = {
  ...
  refresh: (refreshToken: string) =>
    request.get<{ access_token: string }>(
      ENDPOINTS.REFRESH || "/auth/refresh-token",
      {
  ...
  updateMe: (data: FormData) => {
    if (!ENDPOINTS.UPDATE_ME)
      throw new Error("Update me endpoint not configured");
