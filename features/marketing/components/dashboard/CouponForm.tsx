...
export default function CouponForm({ initialData }: CouponFormProps) {
  ...
  const form = useForm<CouponFormValues>({
    ...
    defaultValues: {
      ...
      expires: initialData?.expires
        ? new Date(initialData.expires).toISOString().split("T")[0]
        : "",
      ...
      applyTo === "brands"
        ...
        : applyTo === "categories"
          ...
          : applyTo === "products"
            ...
            : [],
    [applyTo, brandsData, categoriesData, productsData],
  );

  // When editing, restore the visual selectedItems chips from loaded data.
  // We use a ref to ensure this runs only once per applyTo scope change.
  const restoredRef = useRef(false);
  useEffect(() => {
    if (restoredRef.current) return;
    if (!initialData?.applyItems?.length || !options?.length) return;
  ...
  }, [initialData?.applyItems, options]);
