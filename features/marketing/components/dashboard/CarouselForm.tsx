...
import {
  useCreateCarousel,
  useUpdateCarousel,
} from "@/features/marketing/hooks/useCarousel";
...
import {
  CarouselFormValues,
  carouselSchema,
} from "@/features/marketing/marketing.schema";
...
export default function CarouselForm({ initialData }: CarouselFormProps) {
  ...
  // handle image preview   const [imagePreview, setImagePreview] = useState<string | null>(typeof editingBrand?.image === 'string' ? editingBrand.image : editingBrand?.image?.url || null);

  const [previewLg, setImagePreviewLg] = useState<string | null>(
    typeof initialData?.carouselLg === "object"
      ? initialData?.carouselLg?.url || null
      : initialData?.carouselLg || null,
  );
  const [previewMd, setImagePreviewMd] = useState<string | null>(
    typeof initialData?.carouselMd === "object"
      ? initialData?.carouselMd?.url || null
      : initialData?.carouselMd || null,
  );
  const [previewSm, setImagePreviewSm] = useState<string | null>(
    typeof initialData?.carouselSm === "object"
      ? initialData?.carouselSm?.url || null
      : initialData?.carouselSm || null,
  );
  ...
  const {
    ...
    defaultValues: {
      descriptionEn:
        initialData?.description && typeof initialData.description === "object"
          ? (initialData.description as { en: string }).en
          : typeof initialData?.description === "string"
            ? initialData.description
            : "",
      descriptionAr:
        initialData?.description && typeof initialData.description === "object"
          ? (initialData.description as { ar: string }).ar
          : typeof initialData?.description === "string"
            ? initialData.description
            : "",
