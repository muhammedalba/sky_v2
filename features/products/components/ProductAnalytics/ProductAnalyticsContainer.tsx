...
import { useProductStats } from '@/hooks/api/useProductStats';
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold bg-linear-to-r from-primary to-indigo-600 bg-clip-text text-transparent">
            {t('title')}
          </h1>
        ...
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/50 backdrop-blur-sm border border-slate-200 text-slate-600 hover:bg-white transition-all disabled:opacity-50"
        >
        ...
        {isLoading ? (
          ...
          <>
            ...
            <StatCard 
              ...
              className={inventoryStats.stockHealth < 30 ? "ring-2 ring-rose-500/20" : ""}
            />
          </>
        )}
      </div>
      ...
      {!isLoading && stats && stats.lowStockCount > 0 && (
        <div className="rounded-3xl p-6 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 flex items-start gap-4">
          <div className="p-3 bg-amber-100 dark:bg-amber-900/30 rounded-2xl text-amber-600">
            <AlertTriangle className="w-6 h-6" />
          </div>
          <div>
            <h4 className="text-lg font-bold text-amber-900 dark:text-amber-400">Inventory Alert</h4>
