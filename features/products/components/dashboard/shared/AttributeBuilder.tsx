...
const AttributeRow = React.memo(
  ({
    ...
    onUpdate: (
      ...
      val: AttributeDefinition[keyof AttributeDefinition],
    ) => void;
    onRemove: (index: number) => void;
    onAddValue: (
      index: number,
      val: string,
      target: "allowedValues" | "allowedUnits",
    ) => void;
    onRemoveValue: (
      attrIndex: number,
      valIndex: number,
    ...
    const availableUnits =
      attr.name && ATTRIBUTE_CONFIG[attr.name as AllowedAttributeName]
      ...
      <div className="p-4 border border-border/50 rounded-xl space-y-4 bg-background/50">
        <div className="relative flex gap-3 items-center">
          <Select
            ...
            options={ATTRIBUTE_NAME_OPTIONS.map((key) => ({
              value: key,
              label: t(`names.${key}`),
            }))}
            className="h-10 mt-3"
            ...
            <div className="flex gap-2 items-end flex-wrap">
              <Input
                ...
                onChange={(e) =>
                  setInputs((prev) => ({
                    ...prev,
                    str: e.target.value.trim().toLowerCase(),
                  }))
                }
                onKeyDown={(e) =>
                  e.key === "Enter" &&
                  (e.preventDefault(), handleAction("str", "allowedValues"))
                }
                className="rounded-xl h-11 flex-1"
              />
              <Button
                type="button"
                onClick={() => handleAction("str", "allowedValues")}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-s-2 border-primary/20 ps-4 py-1">
              ...
              <div className="flex gap-2 items-end ">
                <Input
                  ...
                  value={inputs.num}
                  onChange={(e) =>
                    setInputs((prev) => ({ ...prev, num: e.target.value }))
                  }
                  onKeyDown={(e) =>
                    e.key === "Enter" &&
                    (e.preventDefault(), handleAction("num", "allowedValues"))
                  }
                  className="rounded-xl h-11 flex-1"
                  icon={CheckIcon}
                />
                <Button
                  type="button"
                  onClick={() => handleAction("num", "allowedValues")}
                  variant="secondary"
                  className="rounded-xl h-11"
                >
                  {t("addNumber")}
              ...
              <div className="flex gap-2  items-end ">
                <div className="relative flex-1">
                  <Select
                    ...
                    options={availableUnits.map((key) => ({
                      value: key.value,
                      label: key.label,
                    }))}
        ...
        <div className="flex flex-col gap-3 pt-2">
          <TagGroup
            label={t("valuesLabel")}
            items={attr.allowedValues}
            onRemove={(vIdx) => onRemoveValue(index, vIdx, "allowedValues")}
            color="primary"
          />
          {attr.type === "number" && (
            <TagGroup
              label={t("unitsLabel")}
              items={attr.allowedUnits}
              onRemove={(vIdx) => onRemoveValue(index, vIdx, "allowedUnits")}
              color="secondary"
            />
          )}
        </div>
 ...
 *
 * A dynamic attribute management UI used in product creation/editing.
 * Allows adding, editing, removing attributes and their allowed values/units.
 *
 * Features:
 * - Add new attributes with default configuration.
 * - Prevent duplicate attribute names.
 * - Auto-detect attribute type from ATTRIBUTE_CONFIG.
 * - Reset allowedValues/allowedUnits when type changes.
 * - Add/remove allowedValues and allowedUnits.
 * - Render each attribute using AttributeRow.
 *
 * Props:
 * @param {AttributeDefinition[]} attributes
 *   The current list of product attributes.
 *
 * @param {(attrs: AttributeDefinition[]) => void} onChange
 *   Callback fired whenever attributes are modified.
 *
 * @example
 * <AttributeBuilder
 *   attributes={[
 *     { id: "1", name: "color", type: "string", allowedValues: ["Red", "Blue"] },
 *     { id: "2", name: "weight", type: "number", allowedValues: [10, 20], allowedUnits: ["kg"] }
 *   ]}
 *   onChange={(newAttrs) => console.log(newAttrs)}
 * />
 *
 * @example
 ...
 * updateAttribute(0, "name", "size");
 *
 * @example
 * // Adding allowed value:
 * modifyList(0, "Large", "add", "allowedValues");
 *
 * @example
 * // Removing allowed unit:
 * modifyList(1, 0, "remove", "allowedUnits");
 */
    ...
    (
      ...
      if (key === "name") {
        ...
        const isDuplicate = attributes.some(
          (attr, i) => i !== index && attr.name === strVal,
        );
        if (isDuplicate) {
          toast.error(
            t("duplicateAttributeError", {
              defaultValue:
                "هذه الخاصية موجودة بالفعل، يرجى إضافة القيم الجديدة للخاصية الحالية بدلاً من إنشاء واحدة جديدة.",
            }),
          );
          return;
        }
      ...
      if (key === "name") {
        ...
        const attrConfig = ATTRIBUTE_CONFIG[strVal as AllowedAttributeName];
      ...
      <div className="space-y-6 p-4">
        {attributes.map((attr, index) => (
          <AttributeRow
            ...
            onAddValue={(idx, val, target) =>
              modifyList(idx, val, "add", target)
            }
            onRemoveValue={(idx, vIdx, target) =>
              modifyList(idx, vIdx, "remove", target)
            }
          />
        ))}
        <Button
          type="button"
          variant="outline"
          className="w-full rounded-xl border-dashed py-6 hover:bg-secondary/50 transition-all"
