...
} from "@/features/shipping/hooks/useShippingProviders";
...
export default function ShippingPage() {
  ...
  const { mutateAsync: deleteProviderAsync, isPending: deleteProviderPending } =
    useDeleteShippingProvider();
  const { mutateAsync: updateProviderAsync, isPending: updateProviderPending } =
    useUpdateShippingProvider();
  ...
  const [editingProvider, setEditingProvider] =
    useState<ShippingProvider | null>(null);
  ...
  const handleToggleStatus = useCallback(
    async (provider: ShippingProvider) => {
      try {
            ...
            <Can permission={Permissions.UPDATE_SHIPPING}>
              <Tooltip content={tButtons("edit")}>
                <Button
                  ...
                  disabled={
                    deleteProviderPending || isLoading || updateProviderPending
                  }
                >
            ...
            <Can permission={Permissions.DELETE_SHIPPING}>
              <Tooltip content={tButtons("delete")}>
                <Button
                  ...
                  onClick={() =>
                    handleDelete(provider._id, getTrans(provider.name))
                  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <Modal
        ...
        title={
          editingProvider
            ? t("editProvider") || "تعديل شركة الشحن"
            : t("createProvider") || "إضافة شركة شحن"
        }
        description={
          t("modalDescription") ||
          "قم بتعبئة بيانات شركة الشحن لتوفيرها للعملاء"
        }
      >
