...
export default function ProductsPage({
  ...
  const { mutateAsync: deleteMutation, isPending: deleteProductPending } =
    useDeleteProduct();
  const { mutateAsync: restoreMutation, isPending: restoreProductPending } =
    useRestoreProduct();
  const {
    mutateAsync: hardDeleteMutation,
    isPending: hardDeleteProductPending,
  } = useHardDeleteProduct();
  const { mutateAsync: updateMutation, isPending: updateProductPending } =
    useUpdateProduct();
        ...
        render: (product: Product) => (
          <div className="flex flex-col gap-2">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                updateMutation({ id: product._id, data: { isActive: checked } })
              }
              disabled={updateProductPending}
      ...
      },
      {
        ...
        render: (product: Product) => (
          <>
            <Badge
              variant="outline"
              className="rounded-xl bg-muted/40 border-none text-muted-foreground/80 text-xs px-3 py-1 group-hover:bg-primary/10 group-hover:text-primary transition-colors"
            >
              {product.category &&
        ...
        render: (product: Product) => {
          if (product.isUnlimitedStock) {
            return (
              <Badge
                variant="default"
                className="rounded-full  p-1.5 font-bold text-xs border-indigo-500/20 text-primary bg-indigo-50/50 dark:bg-indigo-500/10 dark:text-indigo-400 gap-1.5 flex items-center w-fit group/badge hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-all duration-300"
              >
                ...
                <Can permission={Permissions.DELETE_PRODUCT}>
                  <Tooltip content={t("form.hardDelete")}>
                    <Button
                      ...
                      onClick={() =>
                        handleHardDelete(product._id, getTrans(product.title))
                      }
                ...
                <Can permission={Permissions.DELETE_PRODUCT}>
                  <Tooltip content={t("form.softDelete")}>
                    <Button
                      ...
                      className="h-8 w-8 rounded-xl bg-background/50 border-border/40 hover:bg-destructive/10 text-destructive hover:text-destructive/70 hover:border-destructive/20 transition-all"
                      onClick={() =>
                        handleSoftDelete(product._id, getTrans(product.title))
                      }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <EntityDataTable<Product>
        ...
        emptyState={{
          title:
            viewTab === "deleted"
              ? t("form.noDeletedProducts")
              : viewTab === "featured"
                ? t("filters.noFeatured")
                : viewTab === "unlimited_stock"
                  ? t("filters.noUnlimitedStock")
                  : viewTab === "lowStock"
                    ? t("filters.noLowStock")
                    : viewTab === "notActive"
                      ? t("filters.noDisabled")
