...
export default function UsersPage() {
      ...
      openDialog({
        ...
        onConfirm: async () => {
          try {
            ...
            const errorMessage =
              error instanceof Error ? error.message : tMessages("deleteError");
    ...
    () => [
      ...
      },
      {
        ...
        render: (user: User) => {
          ...
          return (
            <Badge
              variant={getRoleBadgeVariant(roleLevel)}
              className="rounded-xl px-3 py-1 text-[10px] font-black uppercase tracking-widest border-none"
            >
              ...
              <Can permission={Permissions.UPDATE_USER}>
                <Tooltip content={t("editUser")}>
                  <Button
                    ...
                    onClick={() =>
                      router.push(`/${locale}/dashboard/users/${user._id}/edit`)
                    }
                    disabled={
                      deleteMutation.isPending ||
                      isLoading ||
  ...
  const filteredUsers = useMemo(() => {
    ...
    return data.data.filter(
      (u: User) => getRoleLevel(u?.role) < currentUserLevel,
    );
  }, [data, currentUserLevel]);

  const isAnyMutationPending =
    deleteMutation.isPending || isLoading || updateMutation.isPending;
