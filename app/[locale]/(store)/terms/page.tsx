...
// Icons are matched to section ids from messages/terms/*.json — keep both in sync.
const SECTION_ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  general: FileTextIcon,
  usage: ListChecksIcon,
  sales: ShoppingCartIcon,
  shipping: TruckIcon,
  returns: RotateCcwIcon,
  intellectualProperty: AwardIcon,
  disclaimer: AlertTriangleIcon,
  governingLaw: ScaleIcon,
  forceMajeure: WarningIcon,
  accessRestrictions: LockIcon,
};
...
export default async function TermsPage({ params }: Props) {
  ...
  // Structured data so search engines can surface this as the site's canonical terms page.
  const structuredData = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: t("meta.title"),
    description: t("meta.description"),
    url: `${env.APP_URL}/${locale}/terms`,
  ...
  return (
    <div className="min-h-screen pt-32 pb-20 bg-background text-foreground transition-colors duration-200">
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-10">
        ...
        {/* Terms sections — native <details> keeps this fully server-rendered
            (no client JS) while still giving an accordion interaction. */}
        <div className="space-y-4">
          {sections.map((section, index) => {
            const Icon = SECTION_ICONS[section.id] ?? FileTextIcon;
            return (
              <ScrollReveal
                key={section.id}
                animation="slide-up"
