...
/**
 * Client Component — contains all interactive state for the products listing.
 * Extracted from page.tsx so the parent Server Component can export generateMetadata.
 */
      ...
      <div className="bg-muted/30 pt-40 py-12 border-b border-border/50">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <h1 className="text-4xl font-black mb-4">
            {locale === 'ar' ? 'جميع المنتجات' : 'Shop All Products'}
          </h1>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            {locale === 'ar'
              ? 'استكشف مجموعتنا المتميزة من المنتجات المصممة للمحترفين.'
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-8 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-muted-foreground">
              {isLoading ? '...' : `${data?.meta?.total || 0} ${locale === 'ar' ? 'منتج' : 'Products'}`}
            </span>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-24">
            ...
            <p className="text-muted-foreground">
              {locale === 'ar' ? 'جرب تعديل كلمات البحث.' : 'Try adjusting your search terms.'}
            </p>
          </div>
        )}
