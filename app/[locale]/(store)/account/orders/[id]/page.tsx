...
import { cn, formatDate, getStatusColor, getPaymentStatusColor } from "@/lib/utils";
import type { FileAsset } from "@/shared/types/file-asset";
...
function getFileUrl(value?: string | FileAsset | null): string | undefined {
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2.5 flex-wrap">
                <span className="font-mono font-black text-lg text-foreground bg-muted px-3 py-1 rounded-lg">
                  #
                  {String(order.invoiceNumber ?? order._id.slice(-6)).padStart(
                    4,
                    "0",
                  )}
                </span>
            ...
            <CardContent className="p-5 flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-semibold text-foreground">
                  {locale === "ar"
                    ? "رمز التحقق من التسليم"
                    : "Delivery verification code"}
                </h4>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {locale === "ar"
                    ? "شارك هذا الرمز مع مندوب التوصيل لتأكيد استلامك للطلب"
                    : "Share this code with the courier to confirm you received the order"}
                </p>
              </div>
              ...
              {order.items?.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="font-bold text-sm text-foreground shrink-0 tabular-nums">
        ...
        {order.couponCode && (
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="font-mono font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded">
              {order.couponCode}
            </span>
          </div>
        )}
