...
function StarRating({ rating, qty }: StarRatingProps) {
...
export function ProductsSection({ d }: ProductsSectionProps) {
  const t = useTranslations('dashboard.productsSection');

  // ─── Derived data (memoized) ────────────────────────────────────────────

  const topProducts          = useMemo(() => d?.topProducts ?? [],                               [d?.topProducts]);
  const orderTopProducts     = useMemo(() => d?.orders?.topProducts ?? [],                       [d?.orders?.topProducts]);
  ...
  const maxSold = useMemo(
    () => Math.max(...displayProducts.map((p) => p.totalSold ?? 0), 1),
    [displayProducts],
  );

  const supplierStatusData = useMemo(
    () =>
      (d?.suppliers?.suppliersByStatus ?? []).map((s) => ({
        name:  s._id ? t('active') : t('inactive'),
        value: s.count,
      })),
    [d?.suppliers?.suppliersByStatus, t],
  );

  const supplierPieData = useMemo(
    () =>
      supplierStatusData.map((s) => ({
        ...s,
        color: s.name === t('active') ? '#22c55e' : '#f59e0b',
      })),
    [supplierStatusData, t],
  );
      ...
      <Card className="border shadow-md bg-background">
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          ...
          <Badge variant="secondary" className="text-xs">
            {t('productsCount', { count: displayProducts.length })}
          </Badge>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {displayProducts.map((product, idx) => {
                ...
                return (
                  <div
                    key={product._id}
                    className="flex items-start gap-3 p-3 rounded-2xl hover:bg-secondary/30 transition-colors group border border-border/40"
                  >
                    <span className="text-sm font-black text-muted-foreground w-5 shrink-0 mt-1">
                      #{idx + 1}
                    </span>
                    ...
                    <div className="relative h-14 w-14 rounded-xl overflow-hidden bg-secondary shrink-0 ring-1 ring-border/50">
                      ...
                      <span
                        className={cn(
                          'absolute top-1 inset-e-1 w-2 h-2 rounded-full ring-1 ring-background',
                        ...
                        {product.variantCount != null && (
                          <span className="text-[10px] text-muted-foreground bg-secondary/60 px-1.5 py-0.5 rounded-md">
                            {product.variantCount !== 1
                              ? t('variantsPlural', { count: product.variantCount })
                              : t('variants', { count: product.variantCount })}
                          </span>
                        )}
                        <span
                          className={cn(
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {product.isUnlimitedStock
                          ? `♾ ${t('unlimited')}`
                          : t('stock', { count: product.stockSummary ?? 0 })}
                      </p>
                    </div>
                  </div>
                );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="border shadow-md bg-background">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              {t('totalSuppliers', { count: d?.suppliers?.totalSuppliers ?? 0 })}
            </CardDescription>
          </CardHeader>
            ...
            <div className="space-y-2">
              {suppliersWithProducts.length === 0 ? (
                ...
                suppliersWithProducts.map((s, i) => (
                  <div
                    key={s.supplierId}
                    className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-secondary/30 hover:bg-secondary/50 transition-colors"
                  >
                    ...
                    <Badge variant="secondary" className="text-[10px]">
                      {t('productsCount', { count: s.totalProducts })}
                    </Badge>
                  </div>
                ))
              )}
