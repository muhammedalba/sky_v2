...
export default async function Sidebar({ mode = 'desktop', className, onNavigate, locale }: SidebarProps) {
  ...
  const navigation: { name: string; href: string; icon: keyof typeof Icons; color: string; permission?: string | string[] }[] = [
    ...
    { name: t('reviews'), href: `/${l}/dashboard/reviews`, icon: 'Star', color: 'warning', permission: Permissions.VIEW_REVIEWS },
    ...
    { name: t('paymentMethods'), href: `/${l}/dashboard/payment-methods`, icon: 'DollarSign', color: 'destructive', permission: Permissions.VIEW_PAYMENT_METHODS },
    ...
    { name: t('notifications'), href: `/${l}/dashboard/notifications`, icon: 'Bell', color: 'warning', permission: Permissions.VIEW_NOTIFICATIONS },
