...
export default async function Sidebar({ mode = 'desktop', className, onNavigate, locale }: SidebarProps) {
  ...
  const navigation: { name: string; href: string; icon: keyof typeof Icons; color: string }[] = [
    ...
    { name: t('shipping'), href: `/${l}/dashboard/shipping`, icon: 'Truck', color: 'primary' },
    { name: t('shippingRates'), href: `/${l}/dashboard/shipping-rates`, icon: 'DollarSign', color: 'success' },
    { name: t('taxes'), href: `/${l}/dashboard/taxes`, icon: 'DollarSign', color: 'warning' },
    { name: t('locations'), href: `/${l}/dashboard/locations`, icon: 'MapPin', color: 'destructive' },
    { name: t('users'), href: `/${l}/dashboard/users`, icon: 'Users', color: 'warning/80' },
    { name: t('roles'), href: `/${l}/dashboard/roles`, icon: 'Shield', color: 'primary' },
