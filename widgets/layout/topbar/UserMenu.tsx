  ...
  return (
    <div className="ps-2 relative group">
      ...
      <button className="flex items-center gap-2 outline-none cursor-default">
        {user?.avatar ? <ImageWithFallback
          src={user?.avatar}
          alt={`${user?.name || 'Admin User'} Logo`}
          width={50}
          height={50}
      ...
      {/* Hover Popover */}
      <div className="absolute inset-e-0 top-full mt-2 w-56 rounded-xl border border-border/60 bg-popover p-2 shadow-lg shadow-black/5 opacity-0 invisible translate-y-1 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-200 z-50">
