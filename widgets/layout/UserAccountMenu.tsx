...
const UserAccountMenu = ({
  ...
  const bottomClass = useMemo(
    () => (dir === "top" ? " inset-e-0 top-full" : " inset-s-0 bottom-full"),
    [dir],
  );
  ...
  //   is admin ?
  const isAdmin =
    user && checkUserPermission(user, Permissions.ACCESS_DASHBOARD);
  ...
  return (
    <div
      ...
      className={cn(
        iconOnly
          ? "bg-transparent m-4 relative"
          : "relative flex items-center gap-3 rounded-2xl border border-border/50 bg-background/50  m-3 p-2 shadow-sm hover:border-primary/30 transition-colors group",
        className,
      )}
    >
      {/* Trigger Button */}
      <button
        ...
        aria-label={
          isLoggedIn ? `User menu for ${user?.name}` : "User account menu"
        }
        className={cn(
        ...
        <div className="flex-1 flex justify-between items-center gap-3">
          <div className=" relative flex flex-col min-w-0">
            ...
            <span className="text-sm font-bold truncate text-foreground leading-tight">
              {truncate(user?.name || "login please", 10)}
            </span>
          ...
          {isLoggedIn ? (
            <>
              <div className="p-3 border-b border-border/30 mb-1 bg-muted/50 ">
                <p className="text-sm font-bold truncate title-gradient">
                  {user?.name}
                </p>
              ...
              <div className="p-3">
                <Link
                  href={isAdmin ? `/dashboard/profile` : `/account`}
                  onClick={closeMenu}
                ...
                <Link
                  href={isAdmin ? `/dashboard/orders` : `/account`}
                  onClick={closeMenu}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-accent/50 text-sm font-medium transition-colors"
                >
